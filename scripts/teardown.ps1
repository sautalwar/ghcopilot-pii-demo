...
$summary = [ordered]@{
    ...
    DemoWorkflowRunsDeleted = 0
    SecretScanningAlertsResolved = 0
}
...
function Update-StepProgress {
    ...
    Write-Progress -Activity 'GitHub Copilot security demo teardown' -Status $Status -PercentComplete $percent
}
...
try {
    ...
    Update-StepProgress -Step 4 -Status 'Deleting workflow runs'
    Write-Step 'Deleting recent workflow runs from demo branches'
    $runs = @(Get-Lines { gh run list --repo $RepoSlug --limit 50 --json databaseId,headBranch --jq '.[] | select(.headBranch | startswith("demo/")) | .databaseId' })

    if ($runs.Count -eq 0) {
        Write-WarningMessage 'No workflow runs from demo branches were found in the last 50 runs.'
    }
    else {
        foreach ($run in $runs) {
            if (Invoke-Checked -Description "Deleting workflow run $run" -Command { gh api "repos/$Owner/$Repo/actions/runs/$run" -X DELETE } -AllowFailure) {
                $summary.DemoWorkflowRunsDeleted++
            }
        }

        Write-Success "Deleted $($summary.DemoWorkflowRunsDeleted) demo workflow run(s)."
    }

    Update-StepProgress -Step 5 -Status 'Resolving demo secret scanning alerts'
    Write-Step 'Resolving open secret scanning alerts created during demos'
    $alerts = @(Get-Lines { gh api --paginate "repos/$Owner/$Repo/secret-scanning/alerts" --jq '.[] | select(.state == "open") | .number' })

    if ($alerts.Count -eq 0) {
        Write-WarningMessage 'No open secret scanning alerts were found.'
    }
    else {
        foreach ($alert in $alerts) {
            if (Invoke-Checked -Description "Resolving secret scanning alert $alert" -Command {
                gh api "repos/$Owner/$Repo/secret-scanning/alerts/$alert" -X PATCH -f state=resolved -f resolution=used_in_tests
            } -AllowFailure) {
                $summary.SecretScanningAlertsResolved++
            }
        }

        Write-Success "Resolved $($summary.SecretScanningAlertsResolved) open secret scanning alert(s)."
    }

    Write-Progress -Activity 'GitHub Copilot security demo teardown' -Completed
    Write-Host "`nFull teardown complete for $RepoSlug." -ForegroundColor Green
    ...
    Write-Progress -Activity 'GitHub Copilot security demo teardown' -Completed
    Write-Host "`nTeardown failed: $($_.Exception.Message)" -ForegroundColor Red
    exit 1
}
finally {
    if ($locationPushed) {
